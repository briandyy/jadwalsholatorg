...
document.addEventListener('DOMContentLoaded', () => {
    ...
    darkModeToggle.addEventListener('change', () => {
        if (darkModeToggle.checked) {
            document.body.classList.add('dark-mode');
            setCookie('darkMode', 'enabled', 999999);
        } else {
            document.body.classList.remove('dark-mode');
            setCookie('darkMode', 'disabled', 999999);
        }
        updateSelectStyles();
    });
    ...
    fetch('kota.json')
        ...
        .then(cities => {
            cities.forEach(city => {
                ...
                if(city.toLowerCase() === defaultCity) {
                    option.selected = true;
                }
                citySelect.appendChild(option);
            });
            loadPrayerTimes(defaultCity, currentYear);
        })
        .catch(error => {
    ...
    citySelect.addEventListener('change', () => {
        ...
        loadPrayerTimes(citySelect.value, selectedYear);
    });

    yearSelect.addEventListener('change', () => {
        const selectedCity = citySelect.value;
        loadPrayerTimes(selectedCity, parseInt(yearSelect.value));
    });
    ...
    function loadPrayerTimes(city, year) {
        ...
        const url = `adzan/${city}/${year}/${monthStr}.json`;
